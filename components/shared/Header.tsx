...
interface HeaderProps {
  ...
  user?: {
    name: string;
    email: string;
    role: "patient" | "doctor" | "admin" | "super_admin" | "receptionist";
    avatarUrl?: string;
  };
  onMenuClick: () => void;
}
...
function notifIcon(type: string | null) {
  ...
  if (type === "subscription") return <CreditCard className="h-4 w-4 text-brand-500" />;
...
export function Header({ title, user, onMenuClick }: HeaderProps) {
  ...
  const getProfileHref = () => {
    ...
    if (displayUser.role === "receptionist") return "/reception/dashboard";
    return "/";
  };
